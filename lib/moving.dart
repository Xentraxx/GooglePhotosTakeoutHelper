...
Stream<int> moveFiles(
  ...
  // Create base folders for organization
  final yourPhotosDir = Directory(p.join(output.path, 'Your Photos'));
  final partnerSharedDir = Directory(p.join(output.path, 'Partner Shared'));
  
  await yourPhotosDir.create(recursive: true);
  await partnerSharedDir.create(recursive: true);

  for (final m in allMediaFinal) {
    // Determine the base output directory based on sharing source
    final baseOutputDir = m.isPartnerShared ? partnerSharedDir : yourPhotosDir;
    ...
    for (final file in nullFirst) {
      ...
      final folder = Directory(
        p.join(
          baseOutputDir.path, // Use the appropriate base directory
  ...
  // Create the albums-info.json if needed
  if (albumBehavior == 'json') {
