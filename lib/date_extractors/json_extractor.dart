...
/// Checks if photo was shared from partner
Future<bool> isPartnerShared(File file, {bool tryhard = false}) async {
  final jsonFile = await _jsonForFile(file, tryhard: tryhard);
  if (jsonFile == null) return false;
  try {
    final data = jsonDecode(await jsonFile.readAsString());
    return data['googlePhotosOrigin']?['fromPartnerSharing'] != null;
  } catch (_) {
    return false;
  }
}
